...
;(function(M,window){
	M.extend({
		...
		getFormatNumber:function(s,n){//格式化数字 例如['13,234,234.89','9,872,980','8,872.10']
			if(this.isNumeric(s)){
				 ...
				 return t.split('').reverse().join('')+(s.indexOf('.')==-1?'':Number('0.'+s.split('.')[1]).toFixed(n||2)).toString().substring(1);
			};
			return s;	
		},
		...
		ui:{
			...
			confirm:{
				defaults:{
					...
					innerBackground:'rgba(255,255,255,1)',
					text:'测试文本',
					direction:'x',
					button:['确定'],
					data:null,
				...
				creatNode:function(param){
					var position={1:'fixed',2:'absolute'},
						...
						inner=M(M.creatlabel(),{
							...
							html:M.isString(param.text)&&M.renderHTML({
								proto:{'class':'mt-confirm-content'},
								html:param.text
							})
						});
				...
				renderButton:function(param){
					...
					for(var i=0,str='',color=['gray','blue','blue'];i<length;i++){
						str+=M.renderHTML({
							name:'a',
							href:'javascript:;',
							proto:{'class':className[1]+' mt-flex-inner '+(length==1?color[1]:color[i])},
							html:param.button[i]
						});
					};
					return this.addEvent(M(M.creatlabel(),{
						'class':className[0]+' mt-flex mt-flex-'+param.direction+' '+param.direction,
						html:str
					}),param);
				},
			...
			lazyload:{
				...
				exec:function(ops,list){
					M.delay(50,function(){
						var o=M(window),x_oSize=o.width(),y_oSize=o.height();
						for(var i=0;i<list.length;i++){
							if(list[i].status){
								continue;
							};
							...
							if((x_offset>=x_distance-x_size&&x_offset<x_oSize+x_distance)&&(y_offset>=y_distance-y_size&&y_offset<y_oSize+y_distance)){
								M.ui.loadImage.init({
									...
									onComplete:function(that,i){
										...
										i==list.length-1&&(list=undefined);
									}
								},target,i);
							};
						};
			 ...
			 * @timeout               number              (可选)请求超时时间                    		默认2000毫秒
			...
			ajax:{
				...
				creat:function(index,status,ops,context,args){
					M.ajax({
						...
						url:ops.url+(ops.url.indexOf('?')==-1?'?':'&')+'t='+M.now(),
						type:ops.type,
						...
						success:function(data){
							M.delay(ops.delay,function(){
								if(!!!data.RETURN){
									M.ui.waiting.dismiss();
									ops.render.enabled&&(this.renderError?this.renderError.change(this.renderError.defaults.container,5):this.render(5,ops,context,args));
									M.isFunction(ops.error)&&ops.error.apply(context||this,['没有数据',5].concat(args));
								}else{
									...
									M.isFunction(ops.success)&&ops.success.apply(context||this,[data.RETURN].concat(args));
								};	
							},this);
						},
							...
							M.delay(ops.delay,function(){
								if(status!='success'){
									if(index<ops.times){
										...
										var text={
											error:{type:2,msg:'请求失败'},
											timeout:{type:3,msg:'请求超时'},
											parsererror:{type:4,msg:'数据格式错误'}
										}[status];
										M.ui.waiting.dismiss();
										ops.render.enabled&&(this.renderError?this.renderError.change(this.renderError.defaults.container,text.type):this.render(text.type,ops,context,args));	
										M.isFunction(ops.error)&&ops.error.apply(context||this,[text.msg,text.type].concat(args));
									};
								};
				...
				render:function(type,ops,context,args){
			...
			error:{
				...
				creat:function(ops,context,args){
					...
					self.change(o,ops.type,function(){
						ops.warp.append(o);	
					});
					return o;
				},
				change:function(obj,type,fn){
					M(M.creatlabel('img'),{
						'class':'mt-error-pic',
						src:M.getNormalPath(type==0||type==1?['base/error_1.png','base/error_2.png'][type]:'base/error_3.png')
					}).preventDefault('mousedown').load(function(){
						obj.children().eq(0).children().html(this.outerHTML+(type!=0&&type!=1?M.renderHTML({
							name:'p',
							proto:{'class':'mar-top-10 mt-error-text'},
							html:{2:'请求失败',3:'请求超时',4:'数据格式错误',5:'没有数据',6:'参数错误'}[type]+'，'+M.renderHTML({
								name:'span',
								proto:{'class':'blue'},
			...
			scrollTop:function(){
				var top=M(M.creatlabel('a'),{
					html:M.renderHTML({
						name:'em',
						proto:{'class':'ui-ico-bg'}
					})+'TOP',
					'class':'ui-go-top',
					href:'javascript:;',
					click:function(){
						M('html,body').animate({scrollTop:0},500,'swing');
					}
				});
				M(document.body).append(top);
				M(window).bind('scroll',function(){
	...
	$('.slider-btn').unbind('click').bind('click',function(){
		$(this).parents('.slider-container').find('.slider-more').slideToggle();	
	});
